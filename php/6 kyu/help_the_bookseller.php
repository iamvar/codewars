...
{
    $sum = [];
    foreach ($listOfArt as $c) {
        $sum[$c[0]] = explode(' ', $c)[1] + ($sum[$c[0]] ?? 0);
    }

    array_walk($listOfCat, function (&$v) use ($sum) {$v = sprintf("($v : %d)", $sum[$v] ?? 0);});
    return empty($listOfArt) ? '' : implode(' - ', $listOfCat);
}
